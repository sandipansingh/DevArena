...
type Phase = "loading" | "auth" | "dashboard" | "queue" | "battle" | "result";
...
function App() {
  ...
  useEffect(() => {
    ...
    getTournaments().then(setTournaments).catch(() => undefined);
  }, [phase]);
  ...
  function startQueue() {
    ...
    socketConnection.on("queue:status", (payload: { status: string }) => {
      setQueueStatus(payload.status);
    });

    socketConnection.on("battle:ready", (payload: BattleReadyPayload) => {
      setBattle(payload);
      setTimeLeft(Math.max(0, Math.floor((payload.endsAt - Date.now()) / 1000)));
      setPhase("battle");
    });

    socketConnection.on("battle:opponent-typing", (payload: { chars: number }) => {
      setOpponentChars(payload.chars);
    });

    socketConnection.on("battle:submission-result", (payload: SubmissionMessage) => {
    ...
    socketConnection.on("battle:finished", (payload: BattleFinishedPayload) => {
      ...
      setPhase("result");
      if (token) {
        getMe(token).then(setUser).catch(() => undefined);
      }
    });
  ...
  const authView = (
    <main className="container auth-layout">
      <section className="brand-panel">
        ...
        <p>
          Practice in live 1v1 battles, watch opponent momentum, and sharpen speed under clock constraints.
        </p>
    ...
    <main className="container app-layout">
      ...
      <section className="battle-grid">
        <article className="card">
          ...
          <p className="hint">Source: {battle?.problem.source === "ai" ? "AI-generated" : "Problem library"}</p>
          ...
          <div className="submission-log">
            {submissions.map((item, index) => (
              <p key={`${item.userId}-${index}`}>
                {item.userId === user?.id ? "You" : "Opponent"} submitted: {item.passed ? "Accepted" : "Wrong Answer"}
  ...
  const resultView = (
    <main className="container app-layout">
      ...
      <section className="card result-card">
        <h3>{result?.winnerId === user?.id ? "Victory" : "Defeat"}</h3>
